...
# 헷갈리는 경우 모든 파이썬 코드에서 reverse_lazy를 써도 무방함
...
#소셜로그인 완료 후 이동할 URL주소를 저장하는 변수 - django 기능
...
AUTHENTICATION_BACKENDS = (
    ...
    #소셜 로그인 정보를 django의 User 모델 클래스에 저장하기 위한 클래스
    'django.contrib.auth.backends.ModelBackend'
    )

#구글 개발자 사이트에서 발급받은 ID, PASSWORD 저장
SOCIAL_AUTH_GOOGLE_PLUS_KEY = '308310929897-to3nuiam73v3s8k6q6dq9v2v0fqhcuio.apps.googleusercontent.com'
SOCIAL_AUTH_GOOGLE_PLUS_SECRET = 'dbXL3qlF6VCF82r6WSCZ8ana'
...
INSTALLED_APPS = [
    ...
    'customlogin',
    'blog',
    #pip install social-auth-app-django (관리자 권한 명령 프롬프트) 
    #명령어로 모듈이 설치 되어있어야 사용가능
    'social_django',
]
...
#클라이언트의 요청으로 저장하는 미디어 파일(이미지, 첨부파일) 설정
...
#BASE_DI : 웹프로젝트가 저장된 경로
#os.path.join(기존경로, 새경로) : 기존경로에 새경로를 붙인 문자열을 생성

#현재 프로젝트 폴더 / files에 클라이언트가 업로드한 파일이 저장되도록 설정
MEDIA_ROOT = os.path.join(BASE_DIR, 'files')
#MEDIA_URL 과  MEDIA_ROOT를 설정한뒤, 메인 URLConf에서 URL과 하드웨어 경로를 매칭하는 작업을 수행해야 함 
